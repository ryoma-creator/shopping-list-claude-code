'use client'
// Shopping list item row — supports swipe-out animation
...
interface Props {
  item: ListItem
  isLeaving?: boolean          // triggers swipe-out animation
  onToggle: (id: string, checked: boolean) => void
  onDeleteRequest: (id: string) => void  // shows confirmation first
}

export function ItemRow({ item, isLeaving, onToggle, onDeleteRequest }: Props) {
  return (
    <div
      className={cn(
        'flex items-center gap-3 px-4 py-3 bg-white rounded-2xl border border-rose-100',
        'transition-opacity duration-200 animate-fall-in',
        item.is_checked && 'opacity-40',
        isLeaving && 'animate-swipe-out pointer-events-none'
      )}
    >
      {/* Check circle button */}
      <button
