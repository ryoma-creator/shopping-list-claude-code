    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="px-4 pt-5 pb-3 flex items-center justify-between shrink-0">
        ...
        <button onClick={() => { setEditItem(undefined); setModalOpen(true) }}
          className="flex items-center gap-1.5 bg-rose-400 hover:bg-rose-500 text-white text-sm font-semibold rounded-xl px-3 py-2 transition-colors">
          <Plus size={16} /> 追加
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-24 space-y-4">
        {items.length === 0 && (
          <EmptyState icon={<Plus size={40} />} title="まだ商品がないよ"
            subtitle="よく買う商品を登録しておくと、お買い物の時にすぐ選べるよ！" />
        )}
